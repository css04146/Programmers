...
class Solution {
    public int solution(int m, int n, String[] board) {
        ...
        return findZeroBlock(m, n, table);
    }
    public int findZeroBlock(int m, int n, char[][] table){
        int answer = 0;
        for(int i = 0; i < m; i++){
            for(int j = 0; j < n; j++){
                if(table[i][j] == '-') answer++;
    ...
    public boolean isBlock(int m, int n, char[][] table){
        ...
        else if(table[m][n] != table[m ][n + 1]) return false;
        else if(table[m][n] != table[m + 1][n + 1]) return false;
        else return true;
    }
